import Row from 'react-bootstrap/Row';

function Sources() {
	return (
		<>
			<Row>
				<p id="sources-header">Sources</p>
			</Row>
			<Row id="sources-content">
				<p>
					<a
						href="https://flickr.com/photos/lschlagenhauf/38494602082/"
						target="_blank"
					>
						“Furggelen afterglow”
					</a>{' '}
					by{' '}
					<a href="https://flickr.com/photos/lschlagenhauf/" target="_blank">
						Lukas Schlagenhauf
					</a>{' '}
					is licensed under{' '}
					<a
						href="https://creativecommons.org/licenses/by-nd/2.0/"
						target="_blank"
					>
						CC BY-ND 2.0
					</a>
					.
				</p>
				<p>
					<a
						href="https://unsplash.com/photos/hoS3dzgpHzw?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText"
						target="_blank"
					>
						Photo
					</a>{' '}
					by{' '}
					<a
						href="https://unsplash.com/@usgs?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText"
						target="_blank"
					>
						USGS
					</a>{' '}
					on Unsplash.
				</p>
				<p>
					<a
						href="https://unsplash.com/photos/x0tB8fltGqU?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText"
						target="_blank"
					>
						Photo
					</a>{' '}
					by{' '}
					<a
						href="https://unsplash.com/@matcfelipe?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText"
						target="_blank"
					>
						Mateus Campos Felipe
					</a>{' '}
					on Unsplash.
				</p>
				<p>
					<a href="https://unsplash.com/photos/D17j_v2EDb0?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText">
						Photo
					</a>{' '}
					by{' '}
					<a href="https://unsplash.com/@mrsrachelmcdermott?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText">
						Rachel McDermott
					</a>{' '}
					on Unsplash.
				</p>
				<p>
					<a href="https://unsplash.com/photos/3eC5n6gHwe8?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText">
						Photo
					</a>{' '}
					by{' '}
					<a href="https://unsplash.com/@jonathanborba?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText">
						Jonathan Borba
					</a>{' '}
					on Unsplash.
				</p>
			</Row>
		</>
	);
}

export default Sources;
